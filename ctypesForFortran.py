...
"""
This module provides a function (ctypesForFortranFactory) which returns a decorator,
...
This file is also an executable script that can generate a signature.
Please, use :code:`ctypesForFortran.py -h` for help.
"""
...
def ctypesForFortranFactory(solib):
    """
    :param solib: shared library already opened (with ctypes) or a filename to use.
    :returns: This function returns a tuple (function, handle). The handle can be used with dlclose.
              The returned function will return a decorator used to call fortran routines or
              functions contained in <solib> using ctypes. The returned function take two arguments:
              prefix and suffix which will be added to the python function name to build the
              name of the function in the shared library. By default prefix is the empty string
              whereas suffix is '_'.

    The actual python function that must be decorated must return the signature of the fortran
    routine. The signature is a tuple. Fisrt element is the actual list of arguments which will
    be used to call the fortran routine.
    ...
    a tuple (type, shape, in_out_status), where:
        ...
        - type must be one of np.str, np.bool, np.float64, np.int64, np.float32, np.int32
        - shape is:

            - None in case of a scalar value (np.bool, np.float64, np.int64, ...)
    ...
    python scalars (for instance (np.float64, None, IN) is the signature for a python
    float variable)
    ...
    Note on strings:
        ...
        - scalars strings are converted from and to unicode; it is certainly
          not totally wanted
    ...
    in inout "foo(a)" will return 'a', and not a copy of 'a', with new values
    except for bool arrays in some circumstances)

    Known limitations:

        - only some types have been tested, others will raise an exception but this
          could normally be extended easily
        - logical arrays and scalars must be declared with KIND=1 in the fortran routine
          as bool elements are 1 byte (and not 1 bit) values
        - fortran assumed-shape arrays, assumed-rank arrays or asterisk
          length strings are not callable
        - there is no support for optional fortran argument. We can use python optional arguments
          but the default python value will be passed to the fortran routine
          (it then will appear as present)
    ...
    Usage:
        ...
        :code:`gfortran -c -fPIC foo.F90 && gfortran -shared -g -o foo.so foo.o`,
        or with ifort:
        :code:`ifort -c -fpic foo.F90 && ifort -shared -g -o foo.so foo.o`)
        ...
        .. code-block:: python
            ...
            #With gfortran, if f_int was inside module 'toto',
            #we would use @ctypesFF(prefix='__toto_MOD_', suffix='')
            @ctypesFF()
            def f_int(KIN):
                return ([KIN],
            ...
            for args in [[kin, kinout*1., pin, pinout, lin, linout, cdin, cdinout, cdain,
                         ...
                         [kin, kinout, pin, pinout, lin, linout, cdin.ljust(500), cdinout, cdain,
                          cdainout, kain, kainout, pain, painout,
                          lain, lainout, kain2], # cdin string too long
        ...
        If everything goes right, the script must execute without any output
    """
    ...
    def ctypesForFortran(prefix="", suffix="_"):
        """
        :param prefix: string that we must put before the python function
                       name to build the name of the function contained
                       in the shared library.
        :param suffix: same as prefix but put after the function name
        :returns: This function returns the decorator to use.
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                if not all([s[0] in [numpy.str, numpy.bool, numpy.int64,
                                     numpy.float64, numpy.int32, numpy.float32] for s in signature]):
                ...
                for s in signature:
                    ...
                    if s[0] == numpy.str and len(s[1]) == 1:
                        ...
                        # According to f2py output for gfortran and ifort,
                        #not a pointer, passed by value!
                        additionalArgs.append(ctypes.c_longlong(s[1][0]))
                        additionalArgtypes.append(ctypes.c_longlong)
                        ...
                        if s[1] is None:
                            ...
                            if s[2] in [IN, INOUT]:
                                ...
                                if s[0] == numpy.bool and (true, false) != (1, 0):
                                    ...
                                    arr[argument == True] = true
                                    arr[argument == False] = false
                                    argument = arr
                                if not argument.flags['F_CONTIGUOUS']:
                                    argument = numpy.asfortranarray(argument)
                            else:
                                argument = numpy.ndarray(expected_shape,
                                                         dtype=effective_dtype, order='F')
                                if s[0] == numpy.str:
                                    argument = numpy.asfortranarray(
                                                 numpy.core.defchararray.ljust(argument, s[1][0]))
                ...
                if ret is not None:
                    ...
                    if ret[1] is None or (ret[0] == numpy.str and len(ret[1]) == 1):
                        ...
                        #    "if second element of returned value signature " + \
                        #    "is not None, it must be a tuple"
                        #if ret[0] == numpy.str:
                    ...
                    if s[2] in [OUT, INOUT]:
                        ...
                        if s[0] == numpy.str and len(s[1]) == 1:
                            ...
                            if s[1] is None:
                                ...
                                #If needed, we could reverse F_CONTIGOUS here
...
def fortran2signature(filename=None, fortran_code=None, as_string=True,
    ...
    """
    This function takes a fortran source code and returns a signature which can be used
    by the decorator described before.

    :param filename:     (optional) filename containing the fortran source code
    :param fortran_code: (optional) fortran source code as string
    :param as_string:    (optional) True to get result as a string, False to get python object
    :param kind_real:    (optional) default kind (specified by compilation options) for real
    :param kind_logical: (optional) default kind (specified by compilation options) for logical
    :param kind_integer: (optional) default kind (specified by compilation options) for integer
    :param prefix:       (optional) used to build the symbol name to use in the shared lib (use %s
                         as a placeholder for the module name)
    :param suffix:       (optional) used to build the symbol name to use in the shared lib (use %s
                         as a placeholder for the module name)
    :param solib:
    :param only:         (optional) By default, signature for all symbols are built, you can limit
                         to one subroutine or function by specifying its name here.
                         When using this function with as_string, this argument becomes mandatory.
    :param \*\*kwargs:   *IN* variables must be passed with keywords in case as_string is False
    :returns: This functions returns the signature as a string (if as_string) or as a python object.
    ...
    Example with the command line:
        :code:`ctypesForFortran.py --solib=./foo.so --suffix="_" --kind_real 8 --kind_integer 8 --kind_logical 1 foo.F90`
        outputs a python script that may be used in place of the signature part of the python code
        given as example in the ctypesForFortranFactory function (except that, in the
        example, order of argument for foo subroutine have been changed).
    ...
    assert len(kwargs) == len(set([k.lower() for k in kwargs])), \
           "fortran variables are case-insensitive"
    assert (not as_string) or solib is not None, \
           "solib is required if as_string"
    ...
    #each item is a dict with keys: module, name, in_var_names (list of in var names),
    #signatures (list of signatures), result (result signature)
    objs = []
            ...
            if len(path) == 1 and only in [None, name]:
                ...
                if len(result_name) > 0:
                    if not result_name.startswith("result"):
                        raise RuntimeError("Something after the function " +
                                           "definition which is not the result?")
                   ...
                   ([current_obj['result_name']] if in_function else []):
                    ...
                    if dtype is None:
                        ...
                        for opt in options:
                            ...
                            if opt.startswith("dimension") and opt[9] in [' ', '\t', '(']:
                                if shape is None:
                                    ...
                                    if as_string:
                                        ...
                                        shape = list(shape)
                                dimensions = [item.strip() for item in
                                              opt[opt.find('(') + 1:opt.find(')')].split(',')]
                                for d in dimensions:
                            ...
                            elif opt.startswith("real") and (len(opt) == 4 or
                                                             opt[4] in [' ', '\t', '(']):
                                dtype = "real"
                                decode_kind = True
                            elif opt.startswith("integer") and (len(opt) == 7 or
                                                                opt[7] in [' ', '\t', '(']):
                                dtype = "integer"
                                decode_kind = True
                            elif opt.startswith("logical") and (len(opt) == 7 or
                                                                opt[7] in [' ', '\t', '(']):
                                dtype = "logical"
                                decode_kind = True
                            elif opt.startswith("character") and (len(opt) == 9 or
                                                                  opt[9] in [' ', '\t', '(']):
                                if as_string:
                                    dtype = "numpy.str"
                                else:
                            ...
                            if decode_kind:
                                ...
                                if '(' in opt:
                                    ...
                                    if not kind.startswith('kind'):
                                        raise RuntimeError("kind specification " +
                                ...
                                if as_string:
                                    ...
                                    if (dtype, kind) not in select:
                                        raise NotImplementedError("This kind is not implemented: " +
                                                                  str((dtype, kind)))
                    ...
                    if as_string:
                        ...
                        if arg == current_obj['result_name']:
                            ...
                            current_obj['signature'][arg] = "(" + dtype + "," + \
                                                            shape + "," + intent +")"
                    else:
                        if arg == current_obj['result_name']:
                            current_obj['signature'][arg] = (dtype, shape)
            ...
            if len(path) == 1 and (in_subroutine or in_function):
                ...
                for arg in current_obj['must_be_in']:
                    if arg not in current_obj['in_var_names']:
                        raise RuntimeError("An argument (" + arg +
                                           ") with INTENT(OUT) has been used for kind or dimension")
                del current_obj['signature'], current_obj['var_names']
                del current_obj['must_be_in'], current_obj['intents']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prefix', type=str, default="",
                        help='prefix to add to the python function name to build ' +
                             'the symbol name as found in the shared lib')
    parser.add_argument('--suffix', type=str, default="",
                        help='suffix to add to the python function name to build ' +
                             'the symbol name as found in the shared lib')
