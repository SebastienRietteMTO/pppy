...
class pppy_lima(pppy.PPPY):
    ...
    def execute(self, previous_state, timestep, timestep_number):
        ...
        if self._options['adjustment']:
            ...
            result = self._paramADJU(NSV, NIJT, NKT, 1, 0, 0, KRR,
                                     HCONDENS, HLAMBDA3, KCARB, KSOA, KSP, ODUST, OSALT, OORILAM,
        ...
        inst = {}
        PTHVREFZIKB = 300.
        PPRCFR = numpy.zeros((NKT, NIJT))
        result = self._param(NSV, NIJT, NKT, 1, 0, HACTCCN, 0, KRR, timestep, False,
                             PRHODREF, exner, ps['dzz'], PTHVREFZIKB, rhodj,
                             ps['P'], KCARB, KSOA, KSP, ODUST, OSALT, OORILAM,
                             True, PDTHRAD,
                             PTHT, PRT, PSVT, PW_NU, PAERO, PSOLORG, PMI, PTHS, PRS, PSVS,
                             PCLDFR, PICEFR, PPRCFR)
        (_, PTHS, PRS, PSVS, inst['c'], _, inst['r'], inst['i'], inst['s'],
